...
def run(N=10, s_ratio=0.1, pss=0.8, pns=0.01, k=2, time_stop=0, dataset="boston", alpha=0.0001, adversarial=False, iteration=1000):
    ...
    start              = 0
    for i in range(N-1):
        buckets_.append((int(start), int(start + units*k)))
        start = start + units
    s                  = int(N*s_ratio)
