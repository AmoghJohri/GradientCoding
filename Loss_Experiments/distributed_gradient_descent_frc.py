...
def run(N=20, s_ratio=0.1, pss=0.8, pns=0.01, k=2, time_stop=0.0005, dataset="boston", alpha=0.01, adversarial=False, iteration=1000):
    N                  = int(N/k)
    ...
    buckets            = merge([[each for i in range(k)] for each in buckets])
    s                  = int(N*s_ratio)
